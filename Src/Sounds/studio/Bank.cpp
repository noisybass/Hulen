...
namespace Sounds{
	...
	bool CBank::loadBank(std::string bankName, std::string fileName)
	{
		...
		*/
		...
		assert(result == FMOD_OK && "Error al cargar el banco. CBank::loadBank");
	...
	{
		assert(_banks->find(bankName) != _banks->end() && "Tiene que existir el banco para poder descargarlo");
		FMOD::Studio::Bank* bank = _banks->at(bankName);

		FMOD_RESULT result = bank->unload();
		assert(result == FMOD_OK && "Error al descargar el banco. CBank::unloadBank");
	...
	{
		FMOD::Studio::Bank* bank = _banks->at(bankName);
		assert(bank && "No existe el sonido para crear el canal");
